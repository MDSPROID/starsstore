...
class Merk extends CI_Controller {
	...
	function __construct(){
		...
		$this->load->model('sec47logaccess/merk_adm');
	...
	function index(){
		...
		$get_merks = $this->merk_adm->get_merk();
		$this->data['get_merk'] = $get_merks;

		$this->load->view('manage/header');
		$this->load->view('manage/merk/index', $this->data);	
		$this->load->view('manage/footer');
		log_helper('merk', ''.$this->data['username'].' ('.$this->data['id'].') Akses Halaman Merk');
	}
	...
	function proses_tambah_merk(){
		...
		if($this->input->post()){
					...
					log_helper('merk', ''.$this->data['username'].' ('.$this->data['id'].') Tambah Merk ('.$target.')');
				}else{
					log_helper('merk', ''.$this->data['username'].' ('.$this->data['id'].') Gagal Tambah Merk ('.$target.')');
				}
	}
	...
	function update_merk(){
		...
		if($this->input->post()){
				...
				log_helper('merk', ''.$this->data['username'].' ('.$this->data['id'].') Update Merk ('.$target.')');
	...
	function merk_dihapuskan(){ // hapus kategori yang dipilih
		...
		log_helper('merk', ''.$this->data['username'].' ('.$this->data['id'].') Menghapus Merk ('.$target.')');
	}

	function delete_select() { // request hapus pada menu pilihan dropdown
			$this->merk_adm->remove_selected();
			log_helper('merk', ''.$this->data['username'].' ('.$this->data['id'].') Hapus Merk');
			redirect('trueaccon2194/merk');
		}
