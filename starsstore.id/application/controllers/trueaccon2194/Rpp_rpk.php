        ...
        if($id1 == "simpantanggal"){
        	...
        	if($baseURL == "https://www.starsstore.id/"){
        		...
        		if($market1 == "semua"){
	        		...
	        		$this->session->set_flashdata('error', ' Peraturan terbaru per juli, sinkron Data harus memilih *semua* (dijadikan 1 kode edp).');
					redirect($this->agent->referrer());
	        	}
        ...
        foreach($list_market as $hx){
       	...
       	if($btn == "detail_barang_terjual"){
	        ...
	        if($status22 == "semua"){
	        	...
	        	$status2 = array('2hd8jPl613!2_^5', '*^56t38H53gbb^%$0-_-', 'Uywy%u3bShi)payDhal', 'ScUuses8625(62427^#&9531(73'); // status batal tidak diikutkan
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', "PENJUALAN TERCLOSING E-COMMERCE & MARKETPLACE"); // Set kolom A1 dengan tulisan "DATA SISWA"
			// Rename sheet
			$objPHPExcel->getActiveSheet()->setTitle('PENJUALAN TERCLOSING');
	    	...
	    	//table header
			$objPHPExcel->getActiveSheet()->getStyle('A3')->getFont()->setBold(TRUE); // Set bold kolom A1
			$heading = array("No","Invoice","Status Pesanan","Toko Pengirim (EDP)","Artikel","Nama Project","Marketplace","Divisi","Merk","Psg","Odv","Harga RIMS (A)","Diskon","Harga Fix","Harga Fix X Psg","Komisi Toko 5%","Selisih Harga Net & Harga RIMS","Harga Penyesuaian dengan RIMS (Untuk Harga Net Yang Melebihi Harga RIMS)");
	        //loop heading
	        $rowNumberH = 8;
		    $colH = 'A';
		    foreach($heading as $h){
		    ...
		    foreach($terjual as $data){
                ...
                //  echo "0"; 
                //}else{ 
                  $diskon = round(($data->retprc - $data->harga_fix) / $data->retprc * 100) * $data->qty;
                //}
		    	...
		    	if($data->harga_before == "" || $data->harga_before == 0){
		        	...
		        	$harga_before = $data->harga_before;
		        	$harga_net = $data->harga_fix;
		        	$diskonx = round(($data->harga_before - $data->harga_fix) / $data->harga_before * 100)*$data->qty;
		        	$diskonrupiah = ($data->harga_before-$data->harga_fix)*$data->qty;
		        }
	        	...
	        	$total_qty += $data->qty;
		    	$total_rims += $data->retprc;
		    	$total_harga_net += $harga_net;
		    	$total_harga_net_qty += $harga_net*$data->qty;
		    	$total_komisi +=  round(($harga_net*5)/100*$data->qty);
		    	if($harga_net > $data->retprc){
		    		$total_selisihx = $harga_net - $data->retprc;
		    		$penyesuaian = $data->retprc;
		    	}else{
		    		$total_selisihx = 0;
		    		$penyesuaian = $data->harga_fix;
		    	}
		        ...
		        $objPHPExcel->getActiveSheet()->setCellValue("D".$baris, $data->sender); // STORE EDP
		        $objPHPExcel->getActiveSheet()->setCellValue("E".$baris, strtoupper($data->artikel)); // ARTIKEL
		        $objPHPExcel->getActiveSheet()->setCellValue("F".$baris, $data->nama_produk); // HARGA RETAIL / SEBELUM DISKON
		        $objPHPExcel->getActiveSheet()->setCellValue("G".$baris, $data->buy_in); // buy_in
		        $objPHPExcel->getActiveSheet()->setCellValue("H".$baris, strtoupper($data->divisi)); // divisi
		        $objPHPExcel->getActiveSheet()->setCellValue("I".$baris, strtoupper($data->merk)); // merk
		        $objPHPExcel->getActiveSheet()->setCellValue("J".$baris, $data->qty); // qty
		        $objPHPExcel->getActiveSheet()->setCellValue("K".$baris, ""); // ODV
		        $objPHPExcel->getActiveSheet()->setCellValue("L".$baris, $data->retprc); // harga RIMS (A) 
		        $objPHPExcel->getActiveSheet()->setCellValue("M".$baris, $diskon."%"); // diskon
		        $objPHPExcel->getActiveSheet()->setCellValue("N".$baris, $harga_net); // harga_net
		        $objPHPExcel->getActiveSheet()->setCellValue("O".$baris, $harga_net*$data->qty); // harga_net * PSG
		        $objPHPExcel->getActiveSheet()->setCellValue("P".$baris, round(($harga_net*5)/100*$data->qty)); // DISKON rupiah
		        $objPHPExcel->getActiveSheet()->setCellValue("Q".$baris, $total_selisihx); // Selisih harga 
		        $objPHPExcel->getActiveSheet()->setCellValue("R".$baris, $penyesuaian); // Selisih harga 
	        ...
	        $objPHPExcel->getActiveSheet()->setCellValue("I".$baris, "TOTAL");
	        $objPHPExcel->getActiveSheet()->setCellValue("J".$baris, $total_qty);
	        $objPHPExcel->getActiveSheet()->setCellValue("K".$baris, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("L".$baris, $total_rims);
	        $objPHPExcel->getActiveSheet()->setCellValue("M".$baris, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("N".$baris, $total_harga_net); 
	        $objPHPExcel->getActiveSheet()->setCellValue("O".$baris, $total_harga_net_qty);
	        $objPHPExcel->getActiveSheet()->setCellValue("P".$baris, $total_komisi);
	        $objPHPExcel->getActiveSheet()->setCellValue("Q".$baris, $total_selisih);
	        $objPHPExcel->getActiveSheet()->setCellValue("R".$baris, $total_penyesuaian);

	        // Apply style row yang telah kita buat tadi ke masing-masing baris (isi tabel)
	      	$objPHPExcel->getActiveSheet()->getStyle('A'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('B'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('C'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('D'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('E'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('F'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('G'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('H'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('I'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('J'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('K'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('L'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('M'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('N'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('O'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('P'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('Q'.$baris)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('R'.$baris)->applyFromArray($style_col);
		    ...
		    // Create second sheet, representing sales data
		    $objPHPExcel->createSheet();
			$objPHPExcel->setActiveSheetIndex(1);

			$objPHPExcel->setActiveSheetIndex(1)->setCellValue('A1', "PENJUALAN ONWAY E-COMMERCE & MARKETPLACE"); // Set kolom A1 dengan tulisan 
			// Rename sheet
			$objPHPExcel->getActiveSheet()->setTitle('PENJUALAN ONWAY');
		    ...
		    $objPHPExcel->getActiveSheet()->setCellValue('B3',"".date('d/m/Y',strtotime($tgl1))."-".date('d/m/Y',strtotime($tgl2))."");
		    $objPHPExcel->getActiveSheet()->setCellValue('B4',$market_title);
		    $objPHPExcel->getActiveSheet()->setCellValue('B5',"Belum Diterima");
		    $objPHPExcel->getActiveSheet()->setCellValue('B6',"Sudah Dibayar");
		    $objPHPExcel->getActiveSheet()->setCellValue('B7',"Tanggal Order");

	    	//table header
			$objPHPExcel->getActiveSheet()->getStyle('A3')->getFont()->setBold(TRUE); // Set bold kolom A1
			$heading2 = array("No","Invoice","Status Pesanan","Toko Pengirim (EDP)","Artikel","Nama Project","Marketplace","Divisi","Merk","Psg","Odv","Harga RIMS (A)","Diskon","Harga Net","Harga Net X Psg","Komisi Toko 5%","Selisih Harga Net & Harga RIMS","Harga Penyesuaian dengan RIMS (Untuk Harga Net Yang Melebihi Harga RIMS)");
	        //loop heading
	        $rowNumberH2 = 8;
		    $colH2 = 'A';
		    ...
		    $onway = $this->rpp_rpk_adm->pendingan_dibulan_lalu_maks_1_tahun_cek($tgl1, $tgl2, $market);
		    ...
		    foreach($onway as $data2){
		    	$no2++;

		    	//if($data->harga_before == "" || $data->harga_before == 0){ 
                //  echo "0"; 
                //}else{ 
                  $diskon = round(($data2->retprc - $data2->harga_fix) / $data2->retprc * 100) * $data2->qty;
                //}

		    	if($data2->harga_before == "" || $data2->harga_before == 0){
		        	$harga_before = "";//$data->harga_fix;
		        	$harga_net = $data2->harga_fix;
		        	$diskonx = 0;
		        	$diskonrupiah = 0;

		        }else{
		        	$harga_before = $data2->harga_before;
		        	$harga_net = $data2->harga_fix;
		        	$diskonx = round(($data2->harga_before - $data2->harga_fix) / $data2->harga_before * 100)*$data2->qty;
		        	$diskonrupiah = ($data2->harga_before-$data2->harga_fix)*$data2->qty;
		        }

		        // STATUS
		        if($data2->status == "2hd8jPl613!2_^5"){
	        		$statusp = "Menunggu Pembayaran";
	        	}else if($data2->status == "*^56t38H53gbb^%$0-_-"){
	        		$statusp = "Pembayaran Diterima";
	        	}else if($data2->status == "Uywy%u3bShi)payDhal"){
	        		$statusp = "Dalam Pengiriman";
	        	}else if($data2->status == "ScUuses8625(62427^#&9531(73"){
	        		$statusp = "Diterima";
	        	}else if($data2->status == "batal"){ 
	        		$statusp = "Dibatalkan";
	        	}

	        	$total_qty2 += $data2->qty;
		    	$total_rims2 += $data2->retprc;
		    	$total_harga_net2 += $harga_net;
		    	$total_harga_net_qty2 += $harga_net*$data2->qty;
		    	$total_komisi2 +=  round(($harga_net*5)/100*$data2->qty);
		    	if($harga_net > $data2->retprc){
		    		$total_selisih2x = $harga_net - $data2->retprc;
		    		$penyesuaian2 = $data2->retprc;
		    	}else{
		    		$total_selisih2x = 0;
		    		$penyesuaian2 = $data2->harga_fix;
		    	}
		    	$total_selisih2 += $total_selisih2x;
                $total_penyesuaian2 += $penyesuaian2;

		        //pemanggilan sesuaikan dengan nama kolom tabel
		        $objPHPExcel->getActiveSheet()->setCellValue("A".$baris2, $no2); // NAMA PRODUK
		        $objPHPExcel->getActiveSheet()->setCellValue("B".$baris2, $data2->invoice); // INVOICE
		        $objPHPExcel->getActiveSheet()->setCellValue("C".$baris2, $statusp); // STATUS
		        $objPHPExcel->getActiveSheet()->setCellValue("D".$baris2, $data2->sender); // STORE EDP
		        $objPHPExcel->getActiveSheet()->setCellValue("E".$baris2, strtoupper($data2->artikel)); // ARTIKEL
		        $objPHPExcel->getActiveSheet()->setCellValue("F".$baris2, $data2->nama_produk); // HARGA RETAIL / SEBELUM DISKON
		        $objPHPExcel->getActiveSheet()->setCellValue("G".$baris2, $data2->buy_in); // buy_in
		        $objPHPExcel->getActiveSheet()->setCellValue("H".$baris2, strtoupper($data2->divisi)); // divisi
		        $objPHPExcel->getActiveSheet()->setCellValue("I".$baris2, strtoupper($data2->merk)); // merk
		        $objPHPExcel->getActiveSheet()->setCellValue("J".$baris2, $data2->qty); // qty
		        $objPHPExcel->getActiveSheet()->setCellValue("K".$baris2, ""); // ODV
		        $objPHPExcel->getActiveSheet()->setCellValue("L".$baris2, $data2->retprc); // harga RIMS (A) 
		        $objPHPExcel->getActiveSheet()->setCellValue("M".$baris2, $diskon."%"); // diskon
		        $objPHPExcel->getActiveSheet()->setCellValue("N".$baris2, $harga_net); // harga_net
		        $objPHPExcel->getActiveSheet()->setCellValue("O".$baris2, $harga_net*$data2->qty); // harga_net * PSG
		        $objPHPExcel->getActiveSheet()->setCellValue("P".$baris2, round(($harga_net*5)/100*$data2->qty)); // komisi
		        $objPHPExcel->getActiveSheet()->setCellValue("Q".$baris2, $total_selisih2x); // selisih harga net & harga rims
		        $objPHPExcel->getActiveSheet()->setCellValue("R".$baris2, $penyesuaian2); // penyesuaian

		      	// Apply style row yang telah kita buat tadi ke masing-masing baris (isi tabel)
		      	$objPHPExcel->getActiveSheet()->getStyle('A'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('B'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('C'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('D'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('E'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('F'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('G'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('H'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('I'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('J'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('K'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('L'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('M'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('N'.$baris2)->applyFromArray($style_row);
		      	...
		      	$objPHPExcel->getActiveSheet()->getStyle('Q'.$baris2)->applyFromArray($style_row);
		      	$objPHPExcel->getActiveSheet()->getStyle('R'.$baris2)->applyFromArray($style_row);
	      	...
	      	//pemanggilan sesuaikan dengan nama kolom tabel
	        $objPHPExcel->getActiveSheet()->setCellValue("A".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("B".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("C".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("D".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("E".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("F".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("G".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("H".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("I".$baris2, "TOTAL");
	        $objPHPExcel->getActiveSheet()->setCellValue("J".$baris2, $total_qty2);
	        $objPHPExcel->getActiveSheet()->setCellValue("K".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("L".$baris2, $total_rims2);
	        $objPHPExcel->getActiveSheet()->setCellValue("M".$baris2, ""); 
	        $objPHPExcel->getActiveSheet()->setCellValue("N".$baris2, $total_harga_net2); 
	        $objPHPExcel->getActiveSheet()->setCellValue("O".$baris2, $total_harga_net_qty2);
	        $objPHPExcel->getActiveSheet()->setCellValue("P".$baris2, $total_komisi2);
	        $objPHPExcel->getActiveSheet()->setCellValue("Q".$baris2, $total_selisih2);
	        $objPHPExcel->getActiveSheet()->setCellValue("R".$baris2, $total_penyesuaian2);

	        // Apply style row yang telah kita buat tadi ke masing-masing baris (isi tabel)
	      	$objPHPExcel->getActiveSheet()->getStyle('A'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('B'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('C'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('D'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('E'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('F'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('G'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('H'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('I'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('J'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('K'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('L'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('M'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('N'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('O'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('P'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('Q'.$baris2)->applyFromArray($style_col);
	      	$objPHPExcel->getActiveSheet()->getStyle('R'.$baris2)->applyFromArray($style_col);
		    ...
		    $objPHPExcel->getActiveSheet()->getColumnDimension('R')->setAutoSize(true);
	      	...
	      	// Redirect output to a client’s web browser (Excel5)
			$filename = urlencode("REPORT_PENJUALAN_&_ONWAY_".$market_title."_".date('d-F-Y',strtotime($tgl1))."-".date('d-F-Y',strtotime($tgl2)).".xls");
			header('Content-Type: application/vnd.ms-excel');
			header('Content-Disposition: attachment;filename="'.$filename.'"');
			header('Cache-Control: max-age=0');
			$objWriter = IOFactory::createWriter($objPHPExcel, 'Excel5');
			$objWriter->save('php://output');
	        log_helper('laporan', ''.$this->data['username'].' ('.$this->data['id'].') Cetak Laporan Produk Terjual & onway (Excel) Marketplace '.$market_title.' Tanggal '.date('d-F-Y',strtotime($tgl1)).'-'.date('d-F-Y',strtotime($tgl2)).' ');
...
// NEW SYNC STARSSTORE WITH DATABASE POS (ONLINE)        	
	        ...
	        //}else{
		        ...
		        foreach($salesdate as $d){
		        	...
		        	if($cekDb->num_rows() == 0){ // jika 0 maka insert
			        	...
			        	foreach($salesproduk as $p){
			        		...
			        		if($p->harga_fix > $p->retprc){
			        			$harga_barang = $p->retprc;
			        		}else{
			        			$harga_barang = $p->harga_fix;
			        		}
		        ...
		        $this->session->set_flashdata('success', 'Data telah disinkron ke database POS 10001 (online)'); //$market
      			redirect($this->agent->referrer());
	        //}
			...
			$pnjpendingblnlalu = 0;
			foreach($pnjlast as $pnjx1){
				$pnjpendingblnlalu += ($pnjx1->qty*$pnjx1->harga_fix);
			}
			$this->data['totalpendinganbulanlalu'] = $pnjpendingblnlalu;
