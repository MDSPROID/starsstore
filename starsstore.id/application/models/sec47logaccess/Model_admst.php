...
class Model_admst extends CI_Model{
	...
	function penj_by_sosmed_dan_mp(){
		$tahun_ini = date('Y');
		$this->db->select('YEAR(order_customer.tanggal_order) as tahun, order_customer.buy_in, SUM(order_product.qty) as jual_pasang');
		$this->db->from('order_customer'); 
		$this->db->join('order_product', 'order_customer.no_order_cus=order_product.no_order_pro','left');
	...
	function penj_by_sosmed_dan_mp_price_calc(){
		$tahun_ini = date('Y');
		$this->db->select('SUM(order_product.qty) as jual_pasang, SUM(order_product.harga_fix) as harga_pasang');
		$this->db->from('order_customer'); 
		$this->db->join('order_product', 'order_customer.no_order_cus=order_product.no_order_pro','left');
		$this->db->where('YEAR(order_customer.tanggal_order)',$tahun_ini);
		$this->db->where('status','ScUuses8625(62427^#&9531(73');
	...
	function penj_by_sosmed_dan_mp_all(){
		//$tahun_ini = date('Y');
		$this->db->select('YEAR(order_customer.tanggal_order) as tahun, order_customer.buy_in, SUM(order_product.qty) as jual_pasang');
		$this->db->from('order_customer'); 
		$this->db->join('order_product', 'order_customer.no_order_cus=order_product.no_order_pro','left');
	...
	function penj_by_sosmed_dan_mp_all_price_calc(){
		//$tahun_ini = date('Y');
		$this->db->select('SUM(order_product.qty) as jual_pasang, SUM(order_product.harga_fix) as harga_pasang');
		$this->db->from('order_product'); 
		$this->db->join('order_customer', 'order_customer.no_order_cus=order_product.no_order_pro','left');
		//$this->db->where('YEAR(order_customer.tanggal_order)',$tahun_ini);
		$this->db->where('status','ScUuses8625(62427^#&9531(73');
		$this->db->group_by('order_customer.buy_in');
		$x = $this->db->get();
	...
	function c_all_toko_on(){
		$this->db->select('*');
		$this->db->from('toko');
		//$this->db->where('id_bdm','1');
		//$this->db->where( array('latitude !=' => '', 'longitude !=' => ''));
	...
	function c_product_total_order(){
		$this->db->select('COUNt(*) as idp'); // hitung semua order selain batal
		$this->db->from('order_customer');
		$this->db->where_not_in('status', 'batal');
		$r = $this->db->get();
		return $r->row()->idp;
