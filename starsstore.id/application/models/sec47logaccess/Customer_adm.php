<?php
class Customer_adm extends CI_Model{ 
	...
	function get_list_point_customer(){ 
		$this->db->select('*');
		$this->db->from($this->table);
		$this->db->order_by('point_terkumpul desc');
		$q = $this->db->get();
		return $q->result();
	}

	function ubah_status_baca($idx, $data_baca){
		$this->db->where('id', $idx);
		$this->db->update('customer', $data_baca);
	}
	...
	function get_list_produk_for_preview($id){ 
		$this->db->select('produk.nama_produk,produk.slug,produk.artikel,produk.keterangan,produk.tags,produk.status,produk.harga_retail,produk.harga_odv,produk.diskon,produk.stok,produk.berat,produk.point,produk.status,produk.parent,produk.tgl_dibuat,produk.tgl_diubah,rt.nama_depan as user1,ty.nama_depan as user2,produk_milik.milik as barang_milik,merk.merk,kat.kategori as kategori, parent_kategori.id_parent, parent_kategori.parent_kategori AS parentnya_produk');
		$this->db->from('produk');
